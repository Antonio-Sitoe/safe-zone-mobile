...
import { ZoneFormData, createZoneSchema, CharacteristicKey } from "@/utils/schemas/create-zone";
...
export const CreateAndEditArea = ({
	...
	const characteristics = watchedCharacteristics ?? ({} as ZoneFormData["characteristics"]);
	const currentDate = watch("date");
	const currentTime = watch("time");

	const characteristicsList = variant === "safe" ? SAFE_CHARACTERISTICS : DANGER_CHARACTERISTICS;
		...
		<KeyboardAvoidingView behavior={"padding"} keyboardVerticalOffset={100}>
			<Actionsheet isOpen={visible} onClose={handleClose}>
				...
				<ActionsheetContent className="bg-white">
					...
					<View className="flex-row items-center justify-center py-4 px-4 border-b border-gray-200">
						<View className="flex-row items-center">
							...
							<Text className="text-secondary-800 font-montserrat font-bold text-lg ml-2">
								{variant === "safe" ? "Criar zona segura" : "Criar zona de perigo"}
							</Text>
						</View>
					</View>
						...
						<Controller
							...
							render={({ field: { onChange, value } }) => (
								<>
									...
									/>
									{errors.location && (
										<Text className="text-red-500 text-sm mt-1">{errors.location.message}</Text>
									)}
								</>
							)}
						/>
					...
					<View className="flex-row gap-2 mb-6">
						<View className="flex-1">
							...
							>
								...
								<Text className="text-base ml-3">{formatDateForDisplay(currentDate)}</Text>
							</TouchableOpacity>
					...
					<View className="mb-6 w-full">
						<Text className="font-semibold text-base mb-3">Descrição do Incidente</Text>
						<Controller
							control={control}
							name="description"
							render={({ field: { onChange, value } }) => (
					...
					<View className="mb-6 w-full">
						<Text className="font-semibold text-base mb-4">Características da área</Text>
									...
									>
										<View
											className={`w-5 h-5 border-2 rounded mr-2 items-center justify-center ${
												isSelected ? "bg-app-primary border-app-primary" : "border-secondary-900/15"
											}`}
										>
					...
					<View className="flex-row px-4 pb-6 pt-4 space-x-4 border-t border-gray-200">
						...
						<TouchableOpacity
							...
							activeOpacity={0.8}
						>
							<Text className="text-secondary-800 text-base font-semibold">Fechar</Text>
						</TouchableOpacity>
					</View>
				</ActionsheetContent>
			</Actionsheet>
