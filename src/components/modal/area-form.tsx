		...
		>
			<View className="h-[70%] bg-white rounded-t-2xl mt-auto">
				<View className="flex-row items-center justify-center py-4 px-4  border-gray-200 relative">
					<TouchableOpacity onPress={onClose} className="absolute left-4 p-2" activeOpacity={0.7}>
				...
				<ScrollView className="flex-1 px-4 py-6" showsVerticalScrollIndicator={false}>
						...
						<View className="space-y-3">
							{data.map((item) => (
								<TouchableOpacity
									key={item.key}
									onPress={() =>
										toggleCharacteristic(item.key as keyof typeof formData.characteristics)
									}
								...
								>
									<View
										className={`w-5 h-5 border-2 mb-1 rounded mr-2 items-center justify-center ${
											formData.characteristics[item.key as keyof typeof formData.characteristics]
									...
									<Text className="font-light -mt-1 text-base">{item.label}</Text>
								</TouchableOpacity>
							))}
						</View>
					</View>
					...
					{loadingMedia && <ActivityIndicator size="small" color="#10b981" className="mb-4" />}
					<ScrollView horizontal showsHorizontalScrollIndicator={false} className="max-h-32 mb-8">
						{formData.media.map((file, index) => (
							<View key={index} className="w-24 h-24 mr-3 relative">
								{["image", "jpeg", "jpg", "png", "webp"].some((type) =>
									file.type.includes(type)
								) ? (
									<Image source={{ uri: file.uri }} className="w-full h-full rounded-lg" />
								) : (
									<>
										<Image
											source={{ uri: file.uri }}
