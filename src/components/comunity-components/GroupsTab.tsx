...
export const GroupsTab = () => {
	...
	const { data: groupsData, isLoading: groupsLoading } = useGetGroupsQuery(userId);
	...
	const handleGroupPress = (groupId: string) => {
		router.push({
			pathname: "/app/community/[groupId]",
			params: { groupId },
		});
	};
	...
	const handleDeleteGroup = (groupId: string, groupName: string) => {
		Alert.alert(
			"Remover Grupo",
			`Tem certeza que deseja remover o grupo "${groupName}"? Todos os contatos deste grupo também serão removidos.`,
			[
				{
					text: "Cancelar",
					style: "cancel",
							...
							{ groupId },
							{
								onSuccess: () => {
									setSuccessMessage({
										...
										message: "O Grupo Foi Removido Da Sua Lista Com Éxito",
									});
									setShowSuccessModal(true);
								},
								onError: (error: any) => {
									setSuccessMessage({
										title: "Erro",
										message: error?.message || "Erro ao remover grupo",
									});
									setShowSuccessModal(true);
								},
							}
		...
		<View className="flex-1">
			{groupsData?.data.length === 0 ? (
				<View className="flex-1 items-center justify-center py-20">
					<Text className="text-center text-gray-500 text-base font-body">Sem grupos</Text>
				</View>
			) : (
				<ScrollView
					className="px-6 pt-6"
