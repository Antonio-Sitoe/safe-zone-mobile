			...
			>
				...
				<View className="w-full px-6 py-3 gap-1">
					<View className="flex-row items-center justify-between">
						<Text className="text-lg font-bold text-gray-900">Zonas monitorizadas</Text>
						<TouchableOpacity onPress={() => refetch()} className="p-2">
							<RefreshCwIcon className="h-4 w-4 text-gray-500" />
						</TouchableOpacity>
					</View>

					<Text className="text-xs text-gray-500">{total} Zonas registadas</Text>
				</View>
					...
					{TAB_OPTIONS.map((tab) => {
						...
						return (
							<Pressable
								...
								className={cn("rounded-full bg-gray-100 px-4 py-2.5", isActive && "bg-blue-600")}
							>
								<Text
									className={cn("text-xs font-semibold text-gray-600", isActive && "text-white")}
								>
									{tab.label}
								</Text>
							</Pressable>
						...
						{isLoading ? (
							...
							<View className="flex-1 items-center justify-center px-6 py-8">
								<Text className="text-center text-sm text-red-600">
									Não foi possível carregar as zonas. Tenta novamente mais tarde.
								</Text>
							</View>
						) : dataSource.length === 0 ? (
							...
							<>
								{dataSource.map((zone, index) => {
									...
									const { label, indicatorClass, labelClass } = getZoneTypeMeta(zone.type);

									const canManage = zone.createdBy && zone.createdBy === currentUserId;
									...
									return (
										...
										>
											<View className="flex-row items-center gap-3">
												<View className="flex-1 gap-1">
													<View className="flex-row items-center gap-2">
														<View className={cn("h-2.5 w-2.5 rounded-full", indicatorClass)} />
														<Text className="text-base font-bold text-gray-900">
															{zone.slug || "Zona perigosa"}
														</Text>
													</View>
													<Text className={cn("text-xs font-medium", labelClass)}>{label}</Text>
													{zone.date && zone.hour ? (
											...
											{zone.description ? (
												<Text className="text-sm leading-5 text-gray-700">{zone.description}</Text>
											) : null}
											...
											<View className="flex-row flex-wrap items-center justify-between gap-2">
												...
												>
													<Text className="text-xs font-semibold text-white">Ver no mapa</Text>
												</Pressable>
												...
												{canManage ? (
													<View className="flex-row gap-2">
														...
														>
															<Text className="text-xs font-semibold text-gray-800">Editar</Text>
														</Pressable>
														<Pressable
