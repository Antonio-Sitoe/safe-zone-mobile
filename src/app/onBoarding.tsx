...
export default function OnboardingScreen() {
	...
	const AnimatedTitle = ({ index }: { index: number }) => {
		...
		const style = useAnimatedStyle(() => {
			const opacity = interpolate(distance.value, [0, 0.5], [1, 0], Extrapolation.CLAMP);
			const translateY = interpolate(distance.value, [0, 0.5], [0, -30], Extrapolation.CLAMP);
			const scale = interpolate(distance.value, [0, 0.5], [1, 0.9], Extrapolation.CLAMP);
			return {
				opacity,
	...
	const AnimatedSubtitle = ({ index }: { index: number }) => {
		...
		const style = useAnimatedStyle(() => {
			const opacity = interpolate(distance.value, [0, 0.5], [1, 0], Extrapolation.CLAMP);
			const translateY = interpolate(distance.value, [0, 0.5], [0, 20], Extrapolation.CLAMP);
			return {
				opacity,
	...
	const IllustrationComponent = ({ index }: { index: number }) => {
		...
		const style = useAnimatedStyle(() => {
			const opacity = interpolate(distance.value, [0, 0.7], [1, 0.2], Extrapolation.CLAMP);
			const scale = interpolate(distance.value, [0, 0.7], [1, 0.85], Extrapolation.CLAMP);
			const translateY = interpolate(distance.value, [0, 0.7], [0, -40], Extrapolation.CLAMP);
			const rotate = interpolate(distance.value, [0, 0.7], [0, 5], Extrapolation.CLAMP);
	...
	const BackgroundGradient = ({ index }: { index: number }) => {
		const distance = useDerivedValue(() => Math.abs(progress.value - index));
		const style = useAnimatedStyle(() => {
			const opacity = interpolate(distance.value, [0, 0.8], [0.15, 0], Extrapolation.CLAMP);
			return { opacity };
		});
	...
	const Indicator = ({ index }: { index: number }) => {
		...
		const style = useAnimatedStyle(() => {
			...
			const width = interpolate(derived.value, [0, 0.5, 1], [32, 32, 8], Extrapolation.CLAMP);
			const opacity = interpolate(derived.value, [0, 0.5, 1], [1, 1, 0.4], Extrapolation.CLAMP);
	...
	const AnimatedButton = () => {
		...
		return (
			<Animated.View style={buttonStyle}>
				...
				>
					<Text style={[styles.primaryButtonText, { fontSize: RFValue(17, height) }]}>Começar</Text>
				</TouchableOpacity>
			</Animated.View>
		);
	};
