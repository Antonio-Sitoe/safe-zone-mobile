...
import { useDeleteGroupMutation } from "@/react-query/community/community/groupMutations";
				...
				onPress: () => {
					deleteContactMutation.mutate(
						{ contactId },
						{
							...
							onError: (error: any) => {
								Alert.alert("Erro", error?.message || "Erro ao remover contato");
							},
						}
					);
				},
	...
	const handleDeleteGroup = () => {
		deleteGroupMutation.mutate(
			{ groupId },
			{
				...
				onError: (error: any) => {
					Alert.alert("Erro", error?.message || "Erro ao remover grupo");
				},
			}
		);
	};
			...
			<SafeAreaView className="flex-1 bg-gray-50">
				<View className="flex-1 pt-6 pb-5">
					<HeaderWithMenu
						...
						deleteTitle="Remover Grupo"
						deleteMessage={`Tem certeza que deseja remover o grupo "${group?.name}"? Todos os contatos deste grupo também serão removidos.`}
					/>
